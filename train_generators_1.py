...
class Generator(nn.Module):
    def __init__(self, input_shape, num_residual_blocks):
        ...
        self.attention = SelfAttention(256)  # Add attention at the bottleneck
    ...
    def forward(self, x):
        ...
        for layer in self.model:
            if isinstance(layer, ResidualBlock) and not attention_applied:
                features = self.attention(features)  # Apply attention once before residual blocks
...
class ResidualBlock(nn.Module):
    def __init__(self, in_features):
        ...
        self.block = nn.Sequential(
            ...
            nn.Conv2d(in_features, in_features, 3),
            nn.InstanceNorm2d(in_features),
            nn.LeakyReLU(0.2, inplace=True)  # Add activation here
        )
...
class StyleTransfer:
    ...
    def postprocess_image(self, image_tensor):
        """
        Ensures proper postprocessing of output images
        """
